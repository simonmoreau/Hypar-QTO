...
namespace Bim42HyparQto.BuildingCode
{
  	...
  	public static class CodeDuTravail
    {
        ...
        public static int GetUPNumber(int effectif)
        {
            ...
            else if (20 <= effectif <= 50) { return 2; } //it is actually more complex than that, I am being consevative here
            else if (50 < effectif <= 100) { return 2; }
            else if (100 < effectif <= 200) { return 3; }
            else if (200 < effectif <= 300) { return 4; }
            else if (300 < effectif <= 400) { return 5; }
            else if (400 < effectif <= 500) { return 6; }
            else if (effectif > 500)
            {
                return (int)Math.Ceiling(effectif / 100);
            }
        }
        ...
        public static int GetUPNumber(int effectif)
        {
            ...
            else if (100 < effectif <= 200) { return 2; }
            else if (200 < effectif <= 300) { return 2; }
            else if (300 < effectif <= 400) { return 2; }
            else if (400 < effectif <= 500) { return 2; }
            else if (effectif > 500)
            {
                return 2 + (int)Math.Ceiling((effectif-500) / 500);
            }
        }
    }
}